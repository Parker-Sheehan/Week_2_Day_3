...
// console.log(outter(callBack,10,20))
...
const uniq = (arr, callback) =>{
  ...
  const iterator = set.values()
    for(let i = 0; i < set.size; i++){
      cbArr.push(iterator.next().value)
    }
    return callback(cbArr)
}
...
each(names,(indexParam,itemParam) =>{
  console.log(`The item at index ${itemParam} is ${indexParam}`)
})
...
const getUserById = (objArr, id, callback) => {
